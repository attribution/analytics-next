...
const load_cancellation_1 = require("./load-cancellation");
const analytics_1 = require("./analytics");
const disable_segment_1 = require("./disable-segment");
const createWrapper = (...[createWrapperSettings]) => {
    (0, validation_1.validateSettings)(createWrapperSettings);
    const { shouldDisableSegment, getCategories, shouldLoadSegment, integrationCategoryMappings, pruneUnmappedCategories, shouldEnableIntegration, registerOnConsentChanged, shouldLoadWrapper, } = createWrapperSettings;
    return (analytics) => {
        ...
        const loadWithConsent = async (settings, options) => {
            ...
            try {
                await loadWrapper;
                initialCategories =
                    (await shouldLoadSegment?.(new load_cancellation_1.LoadContext())) ||
                        (await getCategories());
            }
            catch (e) {
                // consumer can call ctx.abort({ loadSegmentNormally: true })
                // to load Segment but disable consent requirement
                if (e instanceof load_cancellation_1.AbortLoadError) {
            ...
            (0, validation_1.validateCategories)(initialCategories);
            ...
            const updateCDNSettings = (cdnSettings) => {
                if (!cdnSettings.remotePlugins) {
                    return cdnSettings;
                }
                return disableIntegrations(cdnSettings, initialCategories, integrationCategoryMappings, shouldEnableIntegration);
            };
            return analyticsService.loadNormally(settings, {
                ...options,
                updateCDNSettings: (0, utils_1.pipe)(updateCDNSettings, options?.updateCDNSettings || ((id) => id)),
 ...
 */
const getConsentCategories = (integration) => {
    if (integration &&
...
const disableIntegrations = (cdnSettings, consentedCategories, integrationCategoryMappings, shouldEnableIntegration) => {
    const { remotePlugins, integrations } = cdnSettings;
    const isPluginEnabled = (creationName) => {
    ...
    const results = Object.keys(integrations).reduce((acc, creationName) => {
...
const createDisableOption = (initialCategories, disable) => {
    if (disable === true) {
        return true;
    }
    return (cdnSettings) => {
        return ((0, disable_segment_1.segmentShouldBeDisabled)(initialCategories, cdnSettings.consentSettings) ||
